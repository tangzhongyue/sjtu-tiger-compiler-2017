...
F_fragList SEM_transProg(A_exp exp){
    ...
    Tr_procEntryExit(main, e.exp, Tr_formals(main));
    return Tr_getResult();
}
	...
	switch(a->kind) {
		...
		case A_seqExp: {
			...
			for(; seq; seq = seq->tail){
				exp = transExp(venv, tenv, seq->head, level, tl);
				ty = exp.ty;
                trexp = Tr_Seq(trexp, exp.exp);
			}
		...
		case A_letExp: {
			...
			for (d = a->u.let.decs; d; d = d->tail)
				e = Tr_Seq(e, transDec(venv, tenv, d->head, level, tl));
	...
	switch(d->kind) {
		...
		case A_functionDec: {
			...
			for(fList = d->u.function; fList; fList = fList->tail){
                ...
                S_enter(venv, f->name, E_FunEntry(Tr_newLevel(level, Temp_newlabel(), formalBools), Temp_newlabel(), formalTys, resultTy));
			}
			for(fList = d->u.function; fList; fList = fList->tail){
				f = fList->head;
				S_beginScope(venv);
